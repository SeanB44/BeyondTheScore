...
### Load Data ###
raw.dat.full<-rbind(
      read.xlsx("./Output/Modeling/FINAL/Week_10_year_2023_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_11_year_2021_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_11_year_2022_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_11_year_2023_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_12_year_2021_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_12_year_2022_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_12_year_2023_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_13_year_2021_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_13_year_2022_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_13_year_2023_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_14_year_2021_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_14_year_2022_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_14_year_2023_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_5_year_2024_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_6_year_2023_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_6_year_2024_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_7_year_2023_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_8_year_2023_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_8_year_2024_Data.xlsx"),
      read.xlsx("./Output/Modeling/FINAL/Week_9_year_2023_Data.xlsx")
      )
###### Data Prep #######
...
cor(mod.dat$difference, mod.dat$postgame_wp_diff)#0.52 (Better but not great)
...
mod.dat<-mod.dat%>%
  ...
  dplyr::select(-c(pct_returning_ppa_home, pct_returning_ppa_away))
cor(mod.dat$difference, mod.dat$returning_ppa_diff)# 0.32 #Consider
...
cor(mod.dat$difference, mod.dat$Pwr_Rk_diff)#0.56

# Predictive PowerRank is all thats worth keeping
mod.dat<-mod.dat%>%
  dplyr::select(-c(Home_PwrRk_home, SOS_PwrRk_home, Last5_PwrRk_home,Pwr_Rk_home,Predictive_PwrRk_home,
...
cor(mod.dat$difference, mod.dat$recruiting_diff)#0.35 (Better but not great)
...
mod.dat<-mod.dat[,c(1:5,16,6:15,17:30,32:ncol(mod.dat),31 )] 
names(mod.dat)
mod.dat<-na.omit(mod.dat)
mod.dat$AP_home<-as.factor(mod.dat$AP_home)
mod.dat$AP_away<-as.factor(mod.dat$AP_away)

## Win_prob_away and _home have linear dependencies, so remove _away
mod.dat<-mod.dat%>%
  dplyr::select(-c(win_prob_away))


###### Modelling ######
set.seed(7)
# set.seed(8)
train<-sample(1:nrow(mod.dat), nrow(mod.dat)*.7, replace = F)
...
# Mean Difference
mean(abs(mod1.test.dat$predicted-mod1.test.dat$difference))#7.33
plot(mod1.test.dat$predicted,mod1.test.dat$difference)

# Mean Absolute PErcentage Difference
mean(abs(mod1.test.dat$predicted-mod1.test.dat$difference)/mod1.test.dat$difference) #-0.17 (17% closer than actual)
...
sum(mod1.test.dat$side)/nrow(mod1.test.dat) #86.3

# Cover Spread
mod1.test.dat$spread<-mod1.test.dat$spread_home #market predicted home-team point difference
mod1.test.dat$cover<-ifelse(mod1.test.dat$difference<=mod1.test.dat$spread,1,0) #if actual difference exceeded spread, home covered
mod1.test.dat$pred_cover<-ifelse(mod1.test.dat$predicted<=mod1.test.dat$spread,1,0) #if predicted difference less than spread
mod1.test.dat$pred_cover_correct<-ifelse(mod1.test.dat$cover==mod1.test.dat$pred_cover,1,0) #if prediction was correct
sum(mod1.test.dat$pred_cover_correct)/nrow(mod1.test.dat) #79.8
xtabs(~pred_cover+cover, mod1.test.dat) # Most common error is Type I (False Negative),  predicting cover when actually no cover

##### TRYING OTHER MODELS - COMMENTING OUT AS DEEMED NOT AS GOOD #########
#
...
which.min(reg.summary.BSS$cp) #Best-13
(mod2.best.cp<-names(which(reg.summary.BSS$which[13,]==TRUE)))
...
(mod2.best.bic<-names(which(reg.summary.BSS$which[8,]==TRUE)))
...
(mod2.best.adjr2<-names(which(reg.summary.BSS$which[15,]==TRUE)))
...
new_var_list_1<-c(mod2.best.bic[-1])  #using common variables (-1 to account for intercept)

###### Model 3: Model on new full Variable list ######
if("AP_away1"%in% new_var_list_1) {
  new_var_list_1<-new_var_list_1[-which(new_var_list_1=="AP_away1")]
  new_var_list_1<-c(new_var_list_1,"AP_away")
}

model3.dat<-mod.dat[train,]%>%
  dplyr::select(game, year, week, school_home, school_away,all_of(new_var_list_1), difference)
...
# mod3.test.dat<-mod3.test.dat%>%
#   rename(AP_away1=AP_away)

mod3.preds<-round(predict(fit.3, mod3.test.dat[, -c(1:5, ncol(mod3.test.dat))] ), 1)
